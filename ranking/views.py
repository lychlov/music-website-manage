...
def rankingView(request):
    ...
    # 歌曲分类列表
    All_list = Song.objects.values('song_album').distinct()
    # 歌曲列表信息
    song_album = request.GET.get('type', '')
    if song_album:
        song_info = Dynamic.objects.select_related('song').filter(song__song_album=song_album).order_by(
